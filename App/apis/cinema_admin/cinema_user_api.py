...
class CinemaUsersResource(Resource):
    ...
    def post(self):
        ...
        if action == USER_ACTION_REGISTER:
            ...
            if get_cinema_user(username):
                abort(400, msg="用户已存在，重新输入用户名")
            if get_cinema_user(phone):
                abort(400, msg="手机已存在，重新输入手机号码")
            cinema_user = CinemaUser()
            cinema_user.username = username
            cinema_user.password = password
            cinema_user.phone = phone
            if cinema_user.username in ADMINS:
                cinema_user.is_verify = 1
